...
def main():
  ...
  st.title("Exploring the acoustic and meta features of albums and songs by David Bowie from 1969 to 2018")
  st.header("Introduction")
  st.write('In this project, we aim to visualize and explore the trends and evolution of acoustic features in David Bowie\'s work from 1969 to 2018.')
  ...
  st.markdown('''
              ...
              <li><a href="#charts">Charts allowing for visualization of acoustic features in the songs and the distributions of such features in various albums</a></li>
              <li><a href="#comparison">Comparison of the acoustic features in David Bowie albums vs. the overall features in other works by decade</a></li>
              <li><a href="#search">An interactive music search feature that allows users to retrieve a list of recommended Bowie songs with selected acoustic features</a></li>
              </ul>
              ''', unsafe_allow_html=True)
  st.write("""
  ...
  sort_by = st.selectbox(
    'prioritize',
    ('energy', 'valence', 'danceability', 'instrumentalness', 'date')
  )
  ...
  sql_query = f"""
    SELECT
      song, artist, album, date, energy, valence, danceability, instrumentalness
    FROM 
      acoustic_features
    WHERE
      ...
      (instrumentalness BETWEEN {instrumentalness[0]} AND {instrumentalness[1]})
  ...
  if len(df.index) == 0:
    ...
    html = "<br>Your top recommendations:<br><br><ol id='recommended-list'>"
    for row in df.values:
      ...
      html += f'<li><span id="song-name">{song}</span><span id="song-details"> - {album} ({date})</span></li>'
